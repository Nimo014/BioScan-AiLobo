...
LANDMARK_NUM = list(range(55))  # [4,9,16,36]
...
def _stage_loss(y_true, y_pred):
    ...
    loss = tf.math.reduce_mean(tf.math.square(resized_y_true - resized_y_pred) * tf.cast(resized_mask, tf.float32),
                               axis=-1)
...
def _shift_image_one(img, label, padding=b'zero'):
    ...
    if padding == b'edge':
        s_img = np.pad(img[get_y[0]:get_y[1], get_x[0]:get_x[1], :],
                       ((img_h - get_y[1], get_y[0]), (img_w - get_x[1], get_x[0]), (0, 0)), mode='edge')
    else:
...
model.save('./saved_model_h5/saved_model_openpose_ear_v1.h5')
